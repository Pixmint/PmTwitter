...
async def send_text_message(
    update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, thread_id: Optional[int] = None, **kwargs
):
    ...
    await context.bot.send_message(
        chat_id=chat_id, text=text, message_thread_id=thread_id, reply_to_message_id=reply_to_message_id, **kwargs
    )
...
async def process_tweet_url(
    ...
    if not tweet:
        await send_text_message(update, context, f"❌ Не удалось распарсить твит: {original_url}", thread_id=thread_id)
        return False
    ...
    try:
        ...
        await send_text_message(update, context, f"❌ Ошибка при отправке: {str(e)[:100]}", thread_id=thread_id)
        return False
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, original_url in enumerate(tweet_urls):
        ...
        success = await process_tweet_url(update, context, original_url, thread_id, comment)
    ...
    chat = update.effective_chat
    if (
        config.REMOVE_MESSAGE_IN_GROUPS
